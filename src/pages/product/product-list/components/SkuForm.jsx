...
const SkuForm = (props) => {
  console.log('SkuForm-props', props);
  const {
    modalVisible,
    onCancel,
    onChange,
  ...
  console.log("SkuForm-skuInit",skuInit);

  const [clickedCancel, setClickedCancel] = useState(false);
  const [loading, setLoading] = useState(false);
  const [index, setIndex] = useState(0);
  const [cacheOriginData, setCacheOriginData] = useState({});
  const [data, setData] = useState(skuInit.skuStockList);
  ...
  const onFinish = (values) => {
    console.log("values",values);//奇怪，不知道如何从value获取值
    console.log(data);//所以从data获取
    const skuList = {pid:skuInit.pid,skuStockList:data};
    const res = dispatch({
      type: 'productAndproductList/updateSku',
      payload: skuList,
    });
    if(res){
