...
const TableList = (props) => {
  ...
  const {
    dispatch,
    authAndauthRole: { roleList, pagination },
  } = props;
  const dataColumns = [
    {
      title: '名称',
   ...
   *
   */
  const queryRole = async (values) => {
    console.log('values', values);
    dispatch({
      type: 'authAndauthRole/queryRole',
      payload: values,
    });
  };
  ...
  return (
    <PageContainer>
      <ProTable
        ...
        request={async (params) => queryRole({ ...params })}
        dataSource={roleList}
        columns={dataColumns}
        rowSelection={{
...
export default connect(({ authAndauthRole }) => ({
  authAndauthRole,
}))(TableList);
