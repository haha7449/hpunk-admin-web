...
const TableList = (props) => {
  ...
  const {
    dispatch,
    orderAndorderDelivery: { orderDeliveryList, orderItemList, pagination },
  } = props;
  const dataColumns = [
    {
      title: '订单编号',
   ...
   *
   */
  const queryOrderDelivery = async (values) => {
    console.log('values', values);
    dispatch({
      type: 'orderAndorderDelivery/queryOrderDelivery',
      payload: values,
    });
  };
  ...
  const handleRemove = async (deleteId, type) => {
    ...
    dispatch({
      type: 'orderAndorderDelivery/deleteOrder',
      payload: {
        ids,
      },
    })
  ...
  const getOrderItemList = async ({ id }) => {
    console.log('id', id);
    dispatch({
      type: 'orderAndorderDelivery/getOrderItemList',
      payload: {
        id,
      },
    })
  ...
  return (
    <PageContainer>
      <ProTable
        ...
        toolBarRender={() => []}
        request={async (params) => queryOrderDelivery({ ...params })}
        dataSource={orderDeliveryList}
        columns={dataColumns}
        rowSelection={{
...
export default connect(({ orderAndorderDelivery }) => ({
  orderAndorderDelivery,
}))(TableList);
